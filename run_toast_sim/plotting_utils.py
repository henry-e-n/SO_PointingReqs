...
def plot_dets(obs, d_start=0, d_end=None, s_start=0, s_end=None, view=None, signal=defaults.det_data, out_dir = "", file_name = ""):
    ...
    ax.legend(loc="best")    
    plt.savefig(os.path.join(out_dir, f"{file_name}_detdata.png"), dpi=300)
    # plt.show()
    plt.close()

def plot_scanning(obs, s_start=0, s_end=None, out_dir=""):
    slc = slice(s_start, s_end, 1)
    ...
    # plt.show()
    plt.savefig(os.path.join(out_dir, "scan_plot.png"), dpi=300)
    plt.close()
